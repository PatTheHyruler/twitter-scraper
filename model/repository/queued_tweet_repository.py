...
class QueuedTweetRepository(BaseRepository[QueuedTweet]):
    ...
    async def get_next(self, amount: int = 10, min_priority: int = -5) -> List[QueuedTweet]:
        stmt = select(self._table).where(self._table.priority >= min_priority).order_by(self._table.priority.desc()).limit(amount)
